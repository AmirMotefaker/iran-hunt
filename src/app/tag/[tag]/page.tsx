...
export async function generateMetadata({ params }: { params: Promise<{ tag: string }> }): Promise<Metadata> {
  ...
  return { title: `ایده‌های ${name}`, description: `تمام ایده‌های ترند جهانی با برچسب ${name} به همراه ترجمه فارسی و تحلیل مشابه ایرانی` };
}
...
export default async function TagPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-12">
      <Link href="/" className="inline-flex items-center gap-1 text-sm font-bold text-gray-600 hover:text-[#ff6154] dark:text-gray-300">
        <ArrowRight size={16} /> بازگشت به خانه
      </Link>

      <div className="mt-4 rounded-3xl bg-gradient-to-l from-indigo-50 via-white to-white p-8 dark:from-indigo-950/40 dark:via-gray-900 dark:to-gray-900">
        <span className="rounded-full bg-indigo-600 px-3 py-1 text-[10px] font-black text-white">برچسب</span>
