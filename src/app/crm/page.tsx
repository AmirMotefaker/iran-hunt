...
export default function CrmPage() {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-12">
      ...
      <p className="mt-1 text-sm text-gray-500">همه کاربران ثبت‌نام‌شده به همراه تاریخ و ساعت شمسی</p>

      {error && <p className="mt-6 rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-600">{error}</p>}

      {rows && (
        <div className="mt-6 overflow-x-auto rounded-3xl border border-gray-100 bg-white shadow-xl">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-right text-xs font-black text-gray-500">
              <tr>
                ...
                <th className="p-4">تاریخ عضویت (شمسی)</th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-gray-400">هنوز ثبت‌نامی نیست</td></tr>}
              {rows.map((r, i) => (
                <tr key={r.email} className="border-t border-gray-50">
                  <td className="p-4 font-bold text-gray-400">{(i + 1).toLocaleString('fa-IR')}</td>
                  <td className="p-4 font-bold">{r.first_name || r.last_name ? `${r.first_name} ${r.last_name}`.trim() : '—'}</td>
