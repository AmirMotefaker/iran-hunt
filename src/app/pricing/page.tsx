...
import { CYCLES, PLANS, priceFor, toman, type CycleId } from '@/lib/plans';
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-16">
        ...
        {CYCLES.map((c) => (
          <button
            ...
            className={`rounded-xl px-4 py-2 text-sm font-bold transition ${
              cycle === c.id ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900' : 'text-gray-500 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-gray-800'
            }`}
          >
            {c.fa}
            {c.discount > 0 && (
          ...
          return (
            <div
              ...
              className={`relative flex flex-col rounded-3xl border p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${
                p.highlight
                  ? 'border-[#ff6154] bg-white ring-2 ring-[#ff6154]/30 dark:bg-gray-900'
                  : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900'
              }`}
            >
              {p.highlight && (
                <span className="absolute -top-3 right-6 rounded-full bg-[#ff6154] px-3 py-1 text-[10px] font-black text-white">
                  پیشنهاد ما ⭐
                </span>
              )}
                ...
                {p.monthly === 0 ? (
                  ...
                  <>
                    ...
                    <p className="mt-1 text-xs font-bold text-gray-500 dark:text-gray-400">
                      {cycle !== 'monthly' ? `پرداخت ${toman(price.total)} تومان برای ${CYCLES.find((c) => c.id === cycle)!.fa}` : 'بدون تعهد'}
                    </p>
                  </>
                )}
              </div>
              ...
              <ul className="mt-6 flex-1 space-y-2.5">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm leading-6 text-gray-700 dark:text-gray-300">
                    <Check size={15} className="mt-1 shrink-0 text-[#ff6154]" /> {f}
                  </li>
                ))}
              </ul>
              ...
              <button
                ...
                className={`mt-6 rounded-2xl px-4 py-3 text-sm font-black transition ${
                  isCurrent
                    ...
                    : p.highlight
                      ? 'bg-[#ff6154] text-white shadow hover:bg-[#e5544a]'
      ...
      <p className="mt-10 text-center text-xs font-bold text-gray-500 dark:text-gray-400">
        🔒 پرداخت امن با زرین‌پال · بدون نیاز به کارت اعتباری بین‌المللی · فاکتور رسمی برای سازمان‌ها
      </p>
    </main>
  );
}
