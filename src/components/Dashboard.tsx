...
export function Dashboard({ data }: { data: DailyData }) {
  ...
  // Top 10 categories (like producthunt.com/categories)
      ...
      <div className="flex gap-1 overflow-x-auto rounded-3xl bg-white p-2 shadow-xl shadow-gray-200/60">
        {PERIODS.map((t) => (
          <button
            ...
            className={`min-w-max flex-1 rounded-2xl px-4 py-3 text-center transition ${
              tab === t.key
                ? 'bg-[#ff6154] text-white shadow-lg shadow-orange-200'
                : 'text-gray-600 hover:bg-gray-50'
            }`}
          >
            <span className="block text-sm font-extrabold" dir="ltr">
              {t.en}
            </span>
            <span
              className={`mt-0.5 block text-[11px] font-medium ${
                tab === t.key ? 'text-white/85' : 'text-gray-400'
              }`}
            >
              {t.fa}
            </span>
      ...
      <div className="mt-6 space-y-6">
        {filtered.length > 0 ? (
          ...
          <div className="rounded-3xl border-2 border-dashed border-gray-300 bg-white p-12 text-center text-gray-500">
            ...
            <p className="mt-2 text-sm">اسکرپ روزانه هنوز اجرا نشده یا این بازه خالی است.</p>
          </div>
        )}
      </div>
    </section>
