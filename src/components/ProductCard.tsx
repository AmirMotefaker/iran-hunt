    ...
    <article className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-lg shadow-gray-200/50 transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="flex flex-wrap gap-2 px-6">
        {product.category.split('•').map((c) => (
          <span key={c} className="rounded-full bg-gradient-to-l from-blue-50 to-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">{c.trim()}</span>
        ))}
      </div>

      {product.faDescription && (
        <p className="mx-6 mt-4 rounded-2xl border border-orange-100 bg-orange-50/60 p-4 text-sm leading-8 text-gray-800">🇮🇷 {product.faDescription}</p>
      )}

      <div className="p-6 pt-4">
        {slug ? (
          <Link href={`/product/${slug}`} className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-4 py-2 text-sm font-bold text-white transition hover:bg-gray-700">
