...
export function StarRating({ slug }: { slug: string }) {
  ...
  const rate = async (v: number) => {
    ...
    if (res.ok) {
      ...
      setAvg(j.avg); setTotal(j.total); setMy(j.myRating);
    }
    setBusy(false);
  };
  ...
  return (
    <div className="flex flex-col items-center gap-2">
        ...
        {[1, 2, 3, 4, 5].map((v) => {
          ...
          return (
            <button
              ...
              onClick={() => rate(v)}
              className="transition hover:scale-110 disabled:cursor-wait"
              title={`امتیاز ${v.toLocaleString('fa-IR')}`}
            >
      ...
      <div className="text-xs font-bold text-gray-500 dark:text-gray-400">
        {avg > 0 ? (
          <span>
            <span className="text-amber-500">{avg.toFixed(1)}</span> از ۵ · {total.toLocaleString('fa-IR')} امتیاز
            {my > 0 && <span className="mr-2 text-[#ff6154]">(امتیاز شما: {my.toLocaleString('fa-IR')})</span>}
          </span>
        ) : (
